...
def house(surface, x, y, width, height):
    ...
    pygame.draw.polygon(house_surface, color[4], \
        [(0, 150), (290, 0), (580, 150)])
    pygame.draw.polygon(house_surface, BLACK, \
        [(0, 150), (290, 0), (580, 150)], 5)
    ...
    pygame.transform.smoothscale(house_surface, \
        (width, height), house_surface_scaled)
...
def sun(surface, x, y, r, n):
    ...
    '''
    angles = []; 
    angles_x_even = [int(np.cos(np.pi * i / n) * r + x)  \
        for i in range(0, 2 * n, 2)] 
    angles_x_odd = [int(np.cos(np.pi * i / n) * 1.2 * r + x) \
        for i in range(1, 2 * n, 2)]
    angles_y_even = [int(np.sin(np.pi * i / n) * r + y) \
        for i in range(0, 2 * n, 2)] 
    angles_y_odd = [int(np.sin(np.pi * i / n) * 1.2 * r + y)  \
        for i in range(1, 2 * n, 2)]
    c = []
    for i in range(n):
...
def tree(surface, x, y, width, height): 
    ...
    pygame.transform.smoothscale(tree_surface, \
        (width, height), tree_surface_scaled)
...
def cloudlet(surface, x, y, width, height):
    ...
    pygame.transform.smoothscale(clouds_surface, \
        (width, height), clouds_surface_scaled)
